<template>
              ...
              <div class="table-responsive p-0">
                <table class="table align-items-center mb-0">
                  ...
                  <tbody>
                    ...
                    <tr v-for="combustibles in paginatedItems" :key="combustibles.id">
                       
                      <td>
                        <div class="d-flex px-2 py-1"> 
                          <div class="d-flex flex-column justify-content-center">
                            <h6 class="mb-0 text-sm">{{ combustibles.name }}</h6>
                      ...
                      <td>
                        <p class="text-xs font-weight-bold mb-0">{{ combustibles.sale_price }}</p>
                      </td>
                      
                      <td class="align-middle text-center">
                        <span class="text-secondary text-xs font-weight-bold"
                          >{{combustibles.purchase_price}}</span
                        >
                      </td>
                      <td class="align-middle text-center">
                        <span class="text-secondary text-xs font-weight-bold"
                          >{{combustibles.measurement}}</span
                        >
                      </td>
  ...
  export default {
    ...
    computed: {
      pageCount() {
        return Math.ceil(this.combustibles.length / this.itemsPerPage);
      },
      paginatedItems() {
        const startIndex = (this.currentPage - 1) * this.itemsPerPage;
        const endIndex = startIndex + this.itemsPerPage;
        return this.combustibles.slice(startIndex, endIndex);
      },
    },
    async mounted() {
      try{
